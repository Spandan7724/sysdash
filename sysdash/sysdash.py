...
def get_disk_info():
    disk = psutil.disk_usage('C:\\')  # Specify the disk drive if necessary
...
def get_packages_info():
    try:
        choco_packages = subprocess.check_output(
            ['choco', 'list', '--local-only'], shell=True).decode().strip().split('\n')
        scoop_packages = subprocess.check_output(
            ['scoop', 'list'], shell=True).decode().strip().split('\n')
        # Adjust for headers
        return f"{len(choco_packages)-1} (choco), {len(scoop_packages)-2} (scoop)"
